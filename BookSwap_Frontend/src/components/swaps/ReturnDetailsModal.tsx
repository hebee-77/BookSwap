...
import { X, RotateCcw, Loader2, Info, KeyRound, PackageCheck, CheckCircle2 } from 'lucide-react';
...
export const ReturnDetailsModal: React.FC<ReturnDetailsModalProps> = ({
  ...
  const { data: returnDetails, isLoading, refetch } = useQuery({
            ...
            <CardContent className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              {isLoading ? (
                ...
                <>
                  ...
                  {/* Actions in Modal if available */}
                  {(returnDetails.canGenerateReturnOtp || returnDetails.canVerifyReturnOtp || returnDetails.canConfirmReceived) && (
                    <div className="flex flex-wrap items-center justify-end gap-2 p-3 rounded-xl border border-primary/20 bg-primary/5">
                      <span className="text-xs font-semibold text-primary mr-auto">Available Action:</span>
                      {returnDetails.canGenerateReturnOtp && (
                        <Button
                          size="sm"
                      ...
                      {returnDetails.canVerifyReturnOtp && (
                        <Button
                          size="sm"
                      ...
                      {returnDetails.canConfirmReceived && (
                        <Button
                          size="sm"
      ...
      {returnDetails && (
        <>
          <ReturnOtpGenerateDialog
            ...
            bookTitle={returnDetails.bookTitle}
            onSuccess={() => refetch()}
          />
